...
namespace greenbar {
  namespace node2 {
    ...
    ERL_NIF_TERM HeaderNode::decorate_term(ErlNifEnv* env, ERL_NIF_TERM term) {
      ...
      enif_make_map_put(env, term, string_to_binary(env, "level"), level, &term);
      return term;
    }
    ...
    ERL_NIF_TERM LinkNode::decorate_term(ErlNifEnv* env, ERL_NIF_TERM term) {
      ...
      enif_make_map_put(env, term, string_to_binary(env, "url"), url, &term);
      enif_make_map_put(env, term, string_to_binary(env, "text"), title, &term);
      return term;
    }
    ...
    ERL_NIF_TERM TableCellNode::decorate_term(ErlNifEnv* env, ERL_NIF_TERM term) {
      ...
      ERL_NIF_TERM align_atom = alignment_to_binary(alignment, env);
      enif_make_map_put(env, term, string_to_binary(env, "alignment"), align_atom, &term);
      return term;
    }
